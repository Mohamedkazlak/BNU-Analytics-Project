...
def required_filter_fields(_scope: AuthScope) -> list[str]:
    """Filters only narrow the authorized view. University President / VP start unfiltered."""
    return []


def apply_scope_defaults(
    scope: AuthScope, filters: AnalyticsFilters
) -> AnalyticsFilters:
    ...
    if (
        scope.role == "senior_management"
        and scope.scope_level == "sector"
        and scope.sector_id
    ):
...
def assert_filters_in_scope(
    scope: AuthScope, filters: AnalyticsFilters, require_complete: bool = True
) -> None:
    ...
    if role == "student":
        if (
            filters.student_id
            and scope.student_id
            and filters.student_id != scope.student_id
        ):
            raise HTTPException(
                status_code=403, detail="Students may only query their own record"
            )
        return
    ...
    if role == "senior_management" and scope.scope_level == "sector":
        if (
            filters.sector_id
            and scope.sector_id
            and filters.sector_id != scope.sector_id
        ):
            raise HTTPException(
                status_code=403,
                detail="Sector filter is outside your authorized sector",
            )
        if (
            filters.college_id
            and scope.sector_id
            and filters.sector_id
            and filters.sector_id != scope.sector_id
        ):
            raise HTTPException(
                status_code=403,
                detail="College filter is outside your authorized sector",
            )

    if role in ("program_director", "academic_affairs"):
        if (
            filters.college_id
            and scope.college_id
            and filters.college_id != scope.college_id
        ):
            raise HTTPException(
                status_code=403,
                detail="College filter is outside your authorized program",
            )
        if (
            filters.sector_id
            and scope.sector_id
            ...
            raise HTTPException(
                status_code=403,
                detail="Sector filter is outside your authorized program",
            )
