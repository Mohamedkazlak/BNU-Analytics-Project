...
export const Route = createFileRoute("/courses")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Average scores by course, section-by-section comparison and an assessment quality indicator.",
      },
      { property: "og:title", content: "Course & Instructor Performance — BNU" },
      {
...
function CoursePerformance() {
  ...
  const courseOptions = [
    ...
    ...data.averageByCourse.map((row) => ({ value: row.course, label: row.course })),
  ];
  const courseRows = data.averageByCourse.filter((row) => curriculum === "all" || row.course === curriculum);
  const sections = data.sections
    .filter((row) => curriculum === "all" || row.course === curriculum)
    .sort((a, b) =>
      sort === "section"
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {courseRows.map((row, i) => (
          <div key={row.course} className="glass-panel p-5">
            <div className="text-[11px] font-medium uppercase tracking-[0.1em] text-ink-soft">{row.course}</div>
            <div className="font-display mt-2 text-3xl font-extrabold text-ink">{row.average}</div>
            <div className="mt-2 flex items-center gap-2">
              <Meter value={row.quality * 10} tone={(["iris", "cyan", "mint", "amber"] as const)[i % 4]!} />
              <span className="text-[11px] font-semibold text-ink-soft">Quality {row.quality}/10</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <Panel title="Average Score per Course">
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={courseRows} margin={{ top: 8, right: 8, bottom: 0, left: -18 }}>
              ...
              <Bar isAnimationActive={false} dataKey="average" name="Average score" radius={[10, 10, 0, 0]} maxBarSize={54}>
                {courseRows.map((row, i) => (
          ...
          <tbody className="divide-y divide-black/5">
            {sections.map((row) => (
              <tr key={row.section} className="bg-white/40">
                <td className="px-4 py-3 font-semibold text-ink">{row.section}</td>
                ...
                <td className="px-4 py-3 text-right text-ink-soft">{row.passRate}%</td>
                <td className="px-4 py-3 text-right">
                  <Badge tone={row.passRate >= 75 ? "pass" : row.passRate >= 60 ? "warn" : "fail"}>
                    {row.passRate >= 75 ? "On track" : row.passRate >= 60 ? "Watch" : "Needs support"}
                  </Badge>
                </td>
              </tr>
            ))}
